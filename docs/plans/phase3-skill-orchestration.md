...
| 步 | 动作 | 可逆性 | 处理 |
|---|---|---|---|
| R1 | 新建 `skills/pm-deep-research/` + SKILL.md + M1 人格 prompt | 加法/可逆 | **立即做**（= M1 开工）|
| R2 | 文本改品牌 FathomX→PM DeepResearch（**仅 live 文档**：ROADMAP/specs/rubric/evaluation/ADR/README/plans/architecture；**archive/ 与 research/_raw/ 历史草稿不动**）| git 可回滚 | 待 Heye 确认 sweep 范围 + 文件名是否一并改 |
| R3 | 移动现有 top-level `prompts/` 进 skill；改 README 品牌 | git 可回滚 | 随 R2 |
| R4 | **移除 vendored Lapis 引擎源码**（`crates/` 61 文件 + Cargo* + lapis.example.toml）| **高 blast-radius**（删整个引擎源码，靠 git 历史/上游可恢复）| **必须 Heye 确认后执行** |

> R4 是唯一不可轻率的步：删的是同步自上游的整个 Rust 引擎。确认前不动。

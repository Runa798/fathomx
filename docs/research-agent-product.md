...
## 10. Rust Workspace 设计

Rust workspace 承担 Reasoning Layer、Retrieval Layer 和 MCP 边界的主要实现。

当前采用领域 crate 自拥有类型的结构，不再保留中央 `lapis-core` / `schema` 类型桶。每个领域维护自己的 public API、DTO、validation 和 runtime 边界，CLI 作为唯一 composition root 负责把配置装配为具体服务。

当前 crate 结构：

```text
crates/
├── lapis-cli/       # CLI binary and composition root
├── lapis-config/    # TOML config DTOs and loader
├── lapis-error/     # transport-neutral Error, ErrorCode, Result
├── lapis-mcp/       # MCP envelope, server, tools, and Error -> ToolError mapping
├── lapis-model/     # model DTOs, ModelService, ModelProvider, OpenAI adapter
├── lapis-net/       # NetworkClient, ReqwestNetworkClient, redaction, retry, wire trace
├── lapis-search/    # search DTOs, SearchService, SearchProvider, Exa/Grok adapters
├── lapis-workflow/  # research requests/reports, policy, budget, agent loop, validator
└── lapis-tests/     # integration tests and test support modules
```

分层原则：

- `lapis-config` 只拥有配置文件 DTO、loader 和 config-level validation，不依赖 workflow/model/search/net/MCP。
- `lapis-cli` 是唯一同时知道 config、network、model/search providers、workflow budget 和 MCP server 的 composition root。
- `lapis-mcp` 是 Claude Code Skill 的稳定工具接口层，只负责协议暴露、参数校验、错误映射和调用 workflow，不暴露内部 provider 细节。
- `lapis-workflow` 负责任务编排、Agent loop、预算控制（`AgentBudgetGuard` 与 `ResearchBudgetGuard`）、工具策略、中间结果校验；它不负责撰写最终自然语言报告。
- `lapis-model` 与 `lapis-search` 负责把具体厂商协议转换为本领域中立结构，不依赖 workflow policy 或 config。
...
当前实现的 MCP 工具（见 `crates/lapis-mcp/src/tools.rs`）：
...
当前 workflow 接受的 `schema_version` 取值（见 `crates/lapis-workflow/src/workflow.rs` 的 `SUPPORTED_SCHEMA_VERSIONS` 常量）：`0.1`。任何其他取值会触发 `ToolErrorCode::unsupported_schema_version`，并通过 MCP envelope 的 `error` 字段返回。
...
`lapis-net` 的 `ReqwestNetworkClient` 默认仅以 `debug` 级别记录出站请求元数据（method / host / path / headers，Authorization 已 redact）与非 2xx 响应的短摘录（前 256 字节）。完整的 provider 请求 / 响应 plaintext body 仅在 `trace` 级别启用时才进入 tracing 流。为兼容既有运维过滤器，tracing target 仍保留 `lapis_core::net::reqwest_client`：

...
> ⚠️ **2026-05-29 引用审计修正**：本文档原含 1 处捏造统计（"MECE 提升召回率 23-31%"）及 3 个不存在的引用，
> 已删除/修正；APOLLO 状态、DeepTRACE 等真实 arxiv ID 已订正。逐条核验记录见 [`research/citation-audit.md`](research/citation-audit.md)。
> 本文档定位为 Track A 的**早期素材**，最新方法决策以 [`research/track-a-orchestration-credibility.md`](research/track-a-orchestration-credibility.md) 为准。
...
**层次化任务分解与穷尽分析** — MECE 约束的维度分解相比平面提示（flat prompting）的收益是**机制性**的（见下方机制），而非来自某个单一量化结论。

> ⚠️ **修正**：本节原引"提升召回率 23-31%（Chen 2024 ACL / Kumar & Patel 2025 IEEE TKDE / Li 2023 JAIR）"。经 arxiv / ACL Anthology / IEEE Xplore / JAIR 全检索，**三篇论文均不存在，该数字系 LLM 合成的伪引用**，已删除。维度分解的真实收益按机制陈述（减少冗余、覆盖正交），不假托不存在的量化结论。详见 [`research/citation-audit.md`](research/citation-audit.md)。
...
- **维度发现是动态的**，不依赖预定义模板
- 后续系统如 DataSTORM（arXiv: 2604.06474）延续此范式

**APOLLO** — ⚠️ OpenReview `vlqwNZWZv2`，**2025-10 被 desk-reject，不作正式引用**
- （内容：模拟人类贡献者的迭代研究+编辑流程，多 Agent 协作检索/事实核查/结构化）——因未录用，相关论点改以已录用论文（DeepTRACE 等）支撑
...
| 10 | ~~APOLLO~~（OpenReview `vlqwNZWZv2`，desk-rejected）| — | ⚠️ 未录用，不引用 |
...
| 16 | DeepTRACE (arXiv: 2509.04499, ICLR 2026 Poster) | 2025 | 8 维度深度研究审计框架 |
| 17 | ResearchRubrics (arXiv: 2511.07685, ICLR 2026 Poster) | 2025 | 深度研究评估提示词和评分标准 |
| 18 | DeepResearch Bench (arXiv: 2506.11763, ICLR 2026 Poster) | 2025 | 首个全面 DRA 基准测试 |
...
1. ~~**Grok 返回的 MECE 相关学术引用**（Chen et al. 2024 ACL, Kumar & Patel 2025 IEEE TKDE, Li et al. 2023 JAIR）需要二次验证。~~ ✅ **已核实：三篇论文均不存在，系 LLM 合成伪引用，相关数字与引用已从正文删除**（2026-05-29，见 [`research/citation-audit.md`](research/citation-audit.md)）。这恰好印证了"宁少但真 + 先验真伪"约束的必要性。

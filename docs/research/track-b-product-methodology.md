...
> **选定方法 + 依据（含真实案例）+ fathomx 落地点**。全部来源已过 [`citation-audit.md`](citation-audit.md)。
...
竞品深度研究采用**五维 MECE 主干**，每维绑定一个被验证有效的方法、并强制证据标准。这五维构成 fathomx **「竞品深度研究」能力（v2.0 首攻）的报告骨架**：
...
- **Porter 五力被系统性误用**：它是**行业结构**工具，不是产品工具；在产品竞品分析里出现几乎都是误用 → fathomx 显式排除出核心。
...
**跨人格质量门（注入所有人格）**：
- **TM-4 认识论状态标注**：每条重要声明标 (a) 有实证—附来源 / (b) 专家观点—注出处 / (c) 假设—给可证伪版 / (d) 推测—显式标注。**这是 fathomx 可信度使命的提示词级落点。**
...
## 需 Heye 拍板的产品判断（非证据可推导，留待确认）

1. **竞争维度与 MECE-6 的关系**：建议「竞品深度研究」能力用 B1 五维作报告骨架，MECE-6 仍为跨能力顶层维度集（见 B1 文末）。是否认可这个分层？
2. **研究人格数量**：B3 收敛到 2 个核心人格（Experience Analyst / Strategist）+ 跨人格质量门。既有 foundations 文档曾设"3 人格 / Deep 级 4-5"。Phase 3 实现时人格集合最终取几个、是否保留 Market/CI Analyst，需结合首攻"竞品研究"定。
3. **ODI 数据来源**：ODI 要 Importance/Satisfaction 客户打分。fathomx 多数场景没有一手问卷——是否接受用"研究证据/市场代理/推断"来估，并以 TM-4 显式标注其证据等级？

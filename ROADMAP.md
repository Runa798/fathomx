# PM DeepResearch Roadmap (v0.1 draft — 2026-05-29)

> 活文档（rolling-wave）。本文件 = **总计划**；每个 Phase / 阶段开始时另写细化计划 `docs/plans/phaseN-*.md`，完成时回写 review 并调整本文件。

## 1. 北极星 / Vision

PM DeepResearch = **产品深度研究的 Skill 层（Layer 1 业务编排）**，MCP 工具能力主要由 **Lapis** 引擎提供（必要时不局限于 Lapis）。
...
| 决策 | 结论 |
|---|---|
| 引擎边界 | **解耦**：PM DeepResearch 只含 skill/prompts/installer；Lapis 作独立 MCP server 消费（上游 `4o3F/Lapis`，AGPL-3.0）。PM DeepResearch 后期提供**一键部署安装 Lapis** 能力 |
| 首攻能力 | **竞品深度研究** = v2.0 第一份真实价值 |
| 分发范围 | **真正公开 OSS 分发** → key/网络故事必须对无基建的普通用户跑通 |
| 许可证 | PM DeepResearch 与 Lapis 同为 **AGPL-3.0**（已带 LICENSE+README 声明；待补 Cargo/组件声明）|

目标形态：
```
使用者机器
 └─ PM DeepResearch（skill 层 + TUI 安装/配置引导）   ← AGPL-3.0
      ├─ 一键安装/拉起 Lapis MCP server        ← 消费上游 4o3F/Lapis（AGPL-3.0）
      └─ 驱动 Layer 1 编排：问题→MECE维度→人格分配→search计划→aspect执行→gap检测→产品决策报告
```
当前状态：repo 仍 vendored 着 Lapis 引擎源码（同步上游带入）。**结构重组（抽离引擎源码、改 README/品牌为 PM DeepResearch）放到 Phase 3 入口**；Phase 1/2 期间保留 vendored 副本当工作参考。
...
- **✅ 状态（2026-05-29）：已签收**（Heye 审计第一轮通过，进 Phase 3）。见 review [`docs/plans/phase2-spec-and-orchestration.md`](docs/plans/phase2-spec-and-orchestration.md#review2026-05-29--主体完成待-heye-确认)。产出：[规格](docs/specs/pm-deep-research-competitive-research-spec.md)·[rubric](docs/evaluation/rubric.md)·[接口](docs/specs/orchestration-interface.md)·[黄金样例](docs/evaluation/golden/running-coach-ai-upgrade.md)(自评 22/24)。关键发现：Lapis 仅 2 工具，v2.0 纯 Skill+prompt 承载产品字段、不改 Rust。
...
- **✅ 状态（2026-05-30）：v2.0 核心达成**。M1（人格+编排 WS-A/B）·M2（13 章报告+证据后处理 WS-C/E）·M3（Skill 入口可运行+Claude-only 降级 WS-D）·M4（端到端实跑 6/6，黄金课题 Strava AI 升级，13 章报告 **rubric 22/24**，B3 较手写黄金 1→2，可复现）全部完成。**退出标准（端到端达 rubric 门槛 + 可复现）已满足。** 引擎 SSE 事件上限本地补丁（仅 `MAX_SSE_EVENTS`）+ 提上游 [4o3F/Lapis#8](https://github.com/4o3F/Lapis/issues/8)。细化计划/发现见 [`docs/plans/phase3-skill-orchestration.md`](docs/plans/phase3-skill-orchestration.md) §3e。
- **结构重组（WS-F）状态**：R1/R2 ✅；R3 README 改品牌 ✅（2026-05-30）；**R3 移 prompts + R4 删 vendored 引擎源码 → 推迟到 Phase 4**（删引擎须先有一键安装器拉上游，否则本地无法跑 Lapis；属"先验证、后抽离"的后半段）。
- **目标**：产出完整 skill 编排，**先把竞品深度研究端到端跑通+验证** = v2.0；其余能力逐版加。
- **含结构重组**：抽离 Lapis 引擎源码、PM DeepResearch 改为 skill 层 + 安装器、README/品牌改为 PM DeepResearch。
...
| **v2.0** | 3 首能力 | 竞品深度研究端到端+验证（✅ 22/24）+ 结构重组（README ✅；引擎抽离移 Phase 4）| 第一份用户价值 |
...
**Phase 2 已落定（2026-05-29）：**
- ✅ 评测 rubric 维度与门槛 → [`docs/evaluation/rubric.md`](docs/evaluation/rubric.md)（12 维 + floor，借 DeepTRACE 8 维 + ResearchRubrics）。
- ✅ 引擎边界：**Lapis 由上游 4o3F 维护，非我们开发**；v2.0 第一版不动引擎，后续 schema 需求**提给 4o3F**（我们提需求、不改源码）。一键安装仍按 ADR-0002 拉取上游构建。

**仍待澄清：**
- ROADMAP / 文档英文化时机（公开 OSS 前）。

**已解决：** Phase 1 的 8 个决策问题（B1-B3 + A1-A5）已齐全并落定；arxiv 示例真伪已核实。Phase 2 全套产出经 Heye 审计第一轮（建设成本维度 + 黄金样例专家行文）通过并签收（2026-05-29）。

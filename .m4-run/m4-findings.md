...
- `MAX_SSE_TOTAL_DATA_BYTES` 保持上游 8MB（峰值仅 1.5MB，远不到）。**注**：曾一度也改 64MB，但撞坏 `lapis-tests::sse_response_rejects_total_data_overflow`（喂~9MB 断言 8MB 拒绝），CI 变红 → 已回退（commit 86847f8）。只留事件数上限那一处补丁。
